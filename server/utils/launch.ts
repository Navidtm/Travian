...
export const launchTravian = async (event: H3Event, path: string) => {
	...
	try {
		...
		// Most handlers close the page when finished. Closing it must also release
		// the browser process; Playwright does not do that automatically.
		page.once('close', () => void browser.close());
		event.node.res.once('close', () => void browser.close());
