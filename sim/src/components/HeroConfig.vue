	...
	<div
		...
		:class="
			modelValue.enabled
				...
				: 'bg-ink-900/50 border-white/[0.06]'
		"
	>
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-2">
				<Crown
					:size="15"
					:class="modelValue.enabled ? 'text-gold-400' : 'text-slate-500'"
				/>
		...
		<div
			v-if="modelValue.enabled && mode === 'attack'"
		...
		>
			<div>
				...
				<input
					...
					@input="
						update({ offBonusPercent: Number(($event.target as HTMLInputElement).value) || 0 })
					"
				/>
			</div>
			<div>
		...
		<div
			v-else-if="modelValue.enabled && mode === 'defense'"
			class="mt-3"
		>
		...
		<p
			...
			>Disabled — no hero present in this battle.</p
		>
	</div>
</template>
