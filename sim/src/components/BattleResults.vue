...
<template>
	<div class="space-y-6">
		<!-- Verdict banner -->
		<div
			class="panel animate-fade-up flex flex-col items-center gap-2 overflow-hidden px-6 py-8 text-center"
		>
			<span
			...
			<p class="text-sm text-slate-500">
				{{ fmt(result.attackPower) }} attack vs {{ fmt(result.effectiveDefensePower) }} effective
				defense
			</p>
		</div>
			...
			<div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
				<div>
					...
					<p class="text-sm font-medium text-slate-200"
						>×{{ result.modifiers.wallMultiplier.toFixed(3) }}</p
					>
				</div>
				<div>
					<p class="text-xs text-slate-500">Residence bonus</p>
					<p class="text-sm font-medium text-slate-200"
						>+{{ fmt(result.modifiers.residenceBonus) }}</p
					>
				</div>
				<div>
					<p class="text-xs text-slate-500">Morale bonus</p>
					<p class="text-sm font-medium text-slate-200"
						>×{{ result.modifiers.moraleBonus.toFixed(3) }}</p
					>
				</div>
				<div>
					<p class="text-xs text-slate-500">M-factor (battle size)</p>
					<p class="text-sm font-medium text-slate-200">{{
						result.modifiers.mFactor.toFixed(4)
					}}</p>
				</div>
			</div>
			<p
				v-if="result.modifiers.moraleBonus > 1"
				class="text-gold-400/90 mt-3 text-xs"
			>
		...
		<div class="panel animate-fade-up px-5 py-4">
			<div class="mb-3 flex items-center justify-between">
				...
				<p class="text-gold-400 text-sm font-semibold">{{ fmt(result.loot.looted) }}</p>
			</div>
			<div class="bg-ink-700 h-2 overflow-hidden rounded-full">
				<div
					class="from-gold-500 to-gold-400 h-full rounded-full bg-gradient-to-r transition-all duration-700"
			...
			<div class="panel animate-fade-up overflow-hidden">
				<div class="panel-header">
					...
					<span class="text-xs text-slate-500"
						>{{ result.attacker.lossPercentage.toFixed(1) }}% lost</span
					>
				</div>
				<div class="overflow-x-auto px-5 py-3">
					<table
						v-if="result.attacker.units.length"
						class="w-full"
					>
						<thead>
							<tr class="text-left text-[11px] tracking-wider text-slate-500 uppercase">
								<th class="pb-2 font-medium">Unit</th>
								<th class="px-3 pb-2 text-right font-medium">Sent</th>
								<th class="px-3 pb-2 text-right font-medium">Lost</th>
								<th class="pb-2 pl-3 text-right font-medium">Survived</th>
							</tr>
						</thead>
						<tbody>
							<UnitResultRow
								v-for="u in result.attacker.units"
								:key="u.unitId"
								:outcome="u"
							/>
						</tbody>
					...
					<p
						...
						>No attacking units were sent.</p
					>
				</div>
			</div>
			...
			<div class="panel animate-fade-up overflow-hidden">
				<div class="panel-header">
					<span class="flex items-center gap-2 text-sm font-semibold text-slate-200">
						<ShieldCheck
							:size="14"
							class="text-emerald2-400"
						/>
						Remaining Defender Units
					</span>
					<span class="text-xs text-slate-500"
						>{{ result.defender.lossPercentage.toFixed(1) }}% lost</span
					>
				</div>
				<div class="overflow-x-auto px-5 py-3">
					<table
						v-if="result.defender.units.length"
						class="w-full"
					>
						<thead>
							<tr class="text-left text-[11px] tracking-wider text-slate-500 uppercase">
								<th class="pb-2 font-medium">Unit</th>
								<th class="px-3 pb-2 text-right font-medium">Garrisoned</th>
								<th class="px-3 pb-2 text-right font-medium">Lost</th>
								<th class="pb-2 pl-3 text-right font-medium">Survived</th>
							</tr>
						</thead>
						<tbody>
							<UnitResultRow
								v-for="u in result.defender.units"
								:key="u.unitId"
								:outcome="u"
							/>
						</tbody>
					...
					<p
						v-else
						class="py-4 text-center text-sm text-slate-500"
						>The village stands undefended.</p
					>
				</div>
			</div>
		</div>
