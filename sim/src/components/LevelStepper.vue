...
const clamped = computed(() =>
	Math.max(props.min, Math.min(props.max, Math.round(props.modelValue || 0))),
);
...
<template>
	<div
		class="bg-ink-900/70 flex items-center overflow-hidden rounded-md border border-white/[0.08]"
	>
		<button
			type="button"
