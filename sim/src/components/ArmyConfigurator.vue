...
const levelStepperTone = computed<'ember' | 'gold'>(() =>
	props.accent === 'ember' ? 'ember' : 'gold',
);
...
<template>
	<div class="divide-y divide-white/[0.05]">
		<div
			...
			:key="unit.id"
			class="flex flex-col gap-2.5 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:gap-3"
		>
			<div class="flex min-w-0 flex-1 items-center gap-2.5">
				<span class="icon-badge border-white/[0.06] bg-white/[0.03] text-slate-500">
					<component
						:is="roleMeta[unit.role]?.icon ?? Swords"
						:size="16"
					/>
				</span>
				<div class="min-w-0">
					...
					<p class="truncate text-[11px] text-slate-500">
						{{ roleMeta[unit.role]?.label }} · Atk {{ unit.attack }} · Def
						{{ unit.defenseInfantry }}/{{ unit.defenseCavalry }}
					</p>
				</div>
			</div>
