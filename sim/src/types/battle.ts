...
/**
 * Per-unit research/upgrade level (Blacksmith for attack, Armory for defense), 0-20, keyed by unit
 * id. Level 0 means unresearched/base stats.
 */
export type UnitLevels = Record<string, number>;

/**
 * Mirrors the real simulator's hero fields exactly: - offBonusPercent: the hero's "Attack bonus"
 * skill (%), multiplies BOTH infantry and cavalry attack totals. - offPoints: the hero's raw
 * "Fighting Strength" when attacking — a flat attack value added to the (non-cavalry) attack pool.
 */
...
/**
 * The real simulator only exposes a defense bonus percentage for the defending hero (no flat
 * defense-strength field) — a defending hero's raw stats depend on persistent hero state that a
 * stateless simulator can't know, so only the trainable "Defense bonus" skill is exposed here.
 */
