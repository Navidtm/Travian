/**
 * Core domain types describing tribes and units. Kept deliberately generic/data-driven so future
 * phases (e.g. new tribes, new unit roles, artifacts, technologies) can extend this without
...
/**
 * Defense in Travian is split into "infantry defense" and "cavalry defense". Every unit has both
 * values; which one applies is determined by the composition of the ATTACKING force (see
 * engine/battleEngine.ts).
 */
export type UnitCategory = 'infantry' | 'cavalry';

/**
 * Role is informational/UI-grouping only — it does not affect combat math directly, but lets the UI
 * group units sensibly and leaves room for future mechanics (e.g. rams reducing wall level,
 * catapults targeting buildings, chiefs/senators reducing loyalty).
 */
