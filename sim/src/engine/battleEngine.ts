 ...
 *
 * This engine was corrected against a real Travian game server engine (a TravianZ-derived
 * codebase's `Battle.php`), replacing Phase 1's approximations with formulas verified line-by-line
 * against that source. Constants (wall percentages, the 1.5 loss exponent, the M-factor curve, the
 * morale curve) are reproduced as found there; artifact/festival/hero- item bonuses are omitted
 * because the source engine itself zeroes them out in simulator context (village/user IDs of 0
 * resolve to neutral multipliers).
 *
 * ## FORMULAS
 *
 * 1. ATTACK. Units split into an infantry-attack pool and a cavalry-attack pool (by unit category —
 *    note some "mounted-sounding" scouts, e.g. the Teuton Scout and Hun Scout Rider, are
 *    infantry-category in the source engine). The attacking hero's "Off. bonus %" multiplies BOTH
 *    pools; the hero's flat "Off. points" (Fighting Strength) is added to the infantry pool only —
 *    this asymmetry is intentional and matches the source engine exactly.
 *
 *    rap = infantryAttack + cavalryAttack
 * 2. DEFENSE. Defending units similarly split into infantry-defense and cavalry-defense totals. The
 *    defending hero's "Def. bonus %" multiplies both totals. Then wall and residence bonuses
 *    apply:
 *
 *    wallMultiplier = (1 + tribeWallBonus) ^ wallLevel residenceBonus = 2 * residenceLevel^2 + 10
 *    (always >= 10)
 *
 *    if wallLevel > 0: infDef *= wallMultiplier; cavDef *= wallMultiplier infDef += residenceBonus *
 *    wallMultiplier cavDef += residenceBonus * wallMultiplier (if the defending army is completely
 *    empty, the wall itself still contributes a small base defense of 10 * wallMultiplier - wallLevel
 *    before the residence bonus is added — an empty village behind a wall is not literally
 *    defenseless) else: infDef += residenceBonus; cavDef += residenceBonus
 *
 *    The final effective defense blends infantry/cavalry defense by the ATTACKER's cavalry attack
 *    share (Travian's real infantry-vs-cavalry defense split):
 *
 *    rdp = (cavalryAttack / rap) * cavDef + (infantryAttack / rap) * infDef
 * 3. MORALE BONUS. Only kicks in when the attacker's village population exceeds the defender's — it
 *    boosts the defender's effective power so that small villages aren't helplessly crushed by much
 *    larger ones:
 *
 *    moraleBonus = attackerPop > defenderPop ? 1 / max(0.667, (defenderPop/attackerPop) ^ (0.2 *
 *    min(1, rap/rdp))) : 1
 * 4. M-FACTOR. A battle-size dependent exponent. Large battles (1000+ total troops on both sides
 *    combined) use a slightly gentler exponent so massive engagements are less winner-take-all:
 *
 *    M = totalTroops >= 1000 ? clamp(2 * (1.8592 - totalTroops^0.015), 1.2578, 1.5) : 1.5
 * 5. CASUALTIES. Normal attacks and Raids use genuinely different formulas (this was the single
 *    biggest gap in Phase 1, which used one formula for both):
 *
 *    NORMAL ATTACK — decisive: the loser is reduced to (up to) 100% losses, while the winner's losses
 *    scale with the power ratio:
 *
 *        winner = rap > rdp
 *        if winner:
 ...
 *
 *    RAID — smooth: losses follow a logistic curve, so even a decisively losing side keeps some
 *    survivors and even a decisive winner takes some losses:
 *
 ...
 * 6. # LOOT. Survivors' total carry capacity caps how many resources can be taken, capped again by what
 *
 * The village has stored. (Deeper raid-vs- normal loot differences — population-based loot caps,
 * resource field destruction — are a later-phase concern; the source engine's own "max bounty"
 * calculation for the simulator uses this same capacity-based cap for both attack types.)
 */
...
/**
 * Blacksmith (attack) / Armory (defense) per-unit upgrade curve, 0-20, reproduced from the source
 * engine exactly:
 *
 ...
 * ============================================================================ ARMY PLANNER
 * ============================================================================ Given a known enemy
 * garrison (e.g. from a spy report — reuses the exact same DefenderConfig the main simulator uses),
 * find the minimum number of each candidate attacking unit (evaluated one unit type at a time, i.e.
 * a pure single-unit-type army) needed to beat it while keeping the attacker's own casualties at or
 * below a target percentage.
 *
 * This deliberately does not attempt to optimize mixed-unit-type armies: doing so changes the
 * infantry/cavalry attack-share that the defense blend depends on (see blendDefense above), turning
 * this into a much harder combinatorial search. Single-unit-type armies are also how most players
 * actually plan real attacks (e.g. "how many Teutonic Knights do I need"), so this is both simpler
 * and practically relevant. Mixed armies are a natural future enhancement.
 *
 * For a fixed unit type, attacker losses are monotonically non-increasing as troop count grows
 * (more attack power only ever helps), so an exponential bracket-then-binary-search finds the
 * minimum satisfying count quickly and reliably without scanning every integer.
 */
