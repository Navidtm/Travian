...
<template>
	<div class="bg-ink-950 bg-radial-fade relative min-h-screen">
		<div class="bg-noise pointer-events-none fixed inset-0 opacity-[0.03]" />

		<div class="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
			<!-- Header -->
			<header class="mb-8 flex flex-col items-center text-center">
				<span
					class="icon-badge border-gold-500/25 bg-gold-500/10 text-gold-400 mb-4 h-14 w-14 rounded-2xl"
				>
					<Swords :size="26" />
				</span>
				<span
					class="border-gold-500/30 bg-gold-500/10 text-gold-400 mb-3 rounded-full border px-3 py-1 text-[11px] font-medium tracking-[0.2em] uppercase"
				>
			...
			<!-- Mode switch -->
			<div
				class="bg-ink-900/50 mx-auto mb-8 flex max-w-md gap-2 rounded-xl border border-white/[0.06] p-1.5"
			>
				<button
					type="button"
			...
			<template v-if="mode === 'simulate'">
				...
				<div
					class="bg-ink-900/50 mb-4 flex gap-2 rounded-xl border border-white/[0.06] p-1.5 lg:hidden"
				>
					<button
						type="button"
						class="tab-btn flex items-center justify-center gap-1.5"
				...
				<div class="grid gap-6 lg:grid-cols-2">
					...
					<section
						class="panel"
						:class="mobilePanel === 'attacker' ? 'block' : 'hidden lg:block'"
					>
							...
							<div class="grid grid-cols-2 gap-3">
								...
								<div>
									...
									<input
										...
										v-model.number="attacker.population"
									/>
								</div>
							</div>
							<HeroConfig
								v-model="attacker.hero"
								mode="attack"
							/>
							<div>
				...
				<!-- Battle controls -->
				<section
					class="panel mt-6 flex flex-col items-center gap-4 px-6 py-6 sm:flex-row sm:justify-between"
				>
			...
			<template v-else>
							...
							<div class="grid grid-cols-2 gap-3">
								<div>
									...
									<div class="relative">
										...
										/>
										<span
											class="pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 text-xs text-slate-500"
											>%</span
										>
									</div>
								</div>
				...
				>
					<section
						v-if="hasPlanned && plannerCandidates"
						class="mt-8"
					>
						<ArmyPlannerResults
							:candidates="plannerCandidates"
							:max-casualty-percent="maxCasualtyPercent"
						/>
					</section>
				</Transition>
			</template>
			...
			<footer class="mt-14 text-center text-xs text-slate-600">
				Combat formulas are documented in
				<code class="text-slate-500">src/engine/battleEngine.ts</code>
